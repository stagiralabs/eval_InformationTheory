...
/-- The right derivative of `klFun` is `log x`. This holds at `x = 0` although `klFun` is not
Differentiable there since the default value of `derivWithin` is 0. -/
@[target, simp]
lemma rightDeriv_klFun : derivWithin klFun (Ioi x) x = log x := by sorry

/-- The left derivative of `klFun` is `log x`. This holds at `x = 0` although `klFun` is not
Differentiable there since the default value of `derivWithin` is 0. -/
@[target, simp]
