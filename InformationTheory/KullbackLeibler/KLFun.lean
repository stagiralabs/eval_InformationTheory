...
@[target]
lemma convexOn_Ioi_klFun : ConvexOn ℝ (Ioi 0) klFun := by exact convexOn_klFun.subset Ioi_subset_Ici_self (convex_Ioi 0)
...
lemma measurable_klFun : Measurable klFun := by exact continuous_klFun.measurable
